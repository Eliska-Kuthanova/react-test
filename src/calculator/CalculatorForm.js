    ...
    return (
        <div>
            <form className="CalculatorForm" onSubmit={handleSubmit}>
                <NumberInput name="a" label="První èíslo" OnChange={e => setA(e.target.value)} />
                <NumberInput name="b" label="Druhé èíslo" OnChange={e => setB(e.target.value)} />
                <Select options={operations} value={operation} onChange={setOperation} />
                <input type="submit" value="Spoèítej" /> 
            </form>
            <Result value={result}/>
        </div>
